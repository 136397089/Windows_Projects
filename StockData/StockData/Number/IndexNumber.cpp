...
vector<string> CNumberManager::ReadDataFromCSVFileRanks(const string& fullFilePath)
{
	...
	if (!file)
	{
		ShowDebugInformationInCon("File open fail.");
		cerr << "file open fail" << endl;
		exit(-1);
	}
	
	while (getline(file, linestring))//第一行是日期,第二行开始才是数据
	{
...
CNumberManager::ReadDataFromCSVFileAndCutRanks(const string& fullFilePath/*,const vector<string>& StringToCut*/, string strCutIndex /*= ","*/)
{
